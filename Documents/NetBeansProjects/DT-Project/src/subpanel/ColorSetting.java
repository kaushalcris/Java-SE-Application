...
public class ColorSetting extends javax.swing.JDialog {

    Dashboard dashboard;

    public ColorSetting(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        dashboard = (Dashboard) parent;
        loadColorTable("SELECT * FROM `color`");
    }

    private void loadColorTable(String query) {
        try {
            ...
            while (results.next()) {
                Vector v = new Vector();
                v.add(results.getString("co_id"));
                v.add(results.getString("co_name"));
                model.addRow(v);
            }
    ...
    private void jTextField2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextField2ActionPerformed
        loadColorTable("SELECT * FROM `color` WHERE `co_name` LIKE '" + jTextField2.getText() + "%'");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            if (color.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Color name is empty", "Warning", JOptionPane.WARNING_MESSAGE);
            } else if (color.length() > 40) {
                JOptionPane.showMessageDialog(this, "Color name should be less than 40 characters", "Warning", JOptionPane.WARNING_MESSAGE);
            } else {
                try {
                    ResultSet results = MySQL.execute("SELECT * FROM `color` WHERE `co_name`='" + color + "'");
                    if (results.next()) {
                        JOptionPane.showMessageDialog(this, "Color already exists", "Warning", JOptionPane.WARNING_MESSAGE);
                    } else {
                        MySQL.execute("UPDATE `color` SET `co_name`='" + color + "' WHERE `co_id`='" + String.valueOf(jTable1.getValueAt(selectedRow, 0)) + "'");
                        resetColorSetting();
                        loadColorTable("SELECT * FROM `color`");
                        JOptionPane.showMessageDialog(this, "Color updated successfully", "Information", JOptionPane.INFORMATION_MESSAGE);
                    }
                } catch (Exception e) {
                    SignIn.log1.warning(e.toString());
                }
        ...
        if (color.isEmpty()) {
            ...
            try {
                ...
                if (results.next()) {
                    ...
                    MySQL.execute("INSERT INTO `color` (`co_name`) VALUES ('" + color + "')");
                    resetColorSetting();
                    loadColorTable("SELECT * FROM `color`");
                    JOptionPane.showMessageDialog(this, "Color added successfully", "Information", JOptionPane.INFORMATION_MESSAGE);
                }
            } catch (Exception e) {
                SignIn.log1.warning(e.toString());
            }
