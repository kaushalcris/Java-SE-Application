...
public class grnView extends javax.swing.JFrame {
    ...
    private void loadGRN() {
        ...
        try {
            ResultSet customerReturnResultSet = MySQL.execute("SELECT * FROM `grn`"
                    + "INNER JOIN `grn_item`"
                    + "ON `grn_item`.`grn_barcode`=`grn`.`barcode`");
            
            DefaultTableModel model = (DefaultTableModel) jTable6.getModel();
            model.setRowCount(0);
            
            while (customerReturnResultSet.next()) {
                String grnbarcode = customerReturnResultSet.getString("barcode");
                String grnDate = customerReturnResultSet.getString("date_time");
                String buyingPrice = customerReturnResultSet.getString("buying_price");
                String totalqty = customerReturnResultSet.getString("total");
                
                Vector vector = new Vector();
                vector.add(grnbarcode);
                vector.add(grnDate);
    ...
    private void initComponents() {
        ...
        kGradientPanel1.setPreferredSize(new java.awt.Dimension(108, 40));
        ...
        jTable6.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Barcode", "GRN Date", "Buying Price", "Total Quantity"
            }
        ) {
    ...
    private void jTextField6KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextField6KeyReleased
        ...
        try {
            ResultSet customerReturnResultSet = MySQL.execute("SELECT * FROM `grn` SELECT * FROM `grn`"
                    + "INNER JOIN `grn_item`"
                    + "ON `grn_item`.`grn_barcode`=`grn`.`barcode`"
                    + "WHERE `grn`.`barcode` LIKE '%" + text + "%'"
                    + "OR `buying_price` LIKE '%" + text + "%'");
    ...
    private void jTable6MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable6MouseClicked
        ...
        if (evt.getClickCount() == 2 && jTable6.getSelectedRow() != -1) {
            ...
            if (returnManagement != null) {
                returnManagement.getjFormattedTextField10().setText(String.valueOf(jTable6.getValueAt(row, 0)));
